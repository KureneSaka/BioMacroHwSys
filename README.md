...
- **管理员端**
  ...
  - 管理学生账号（含课程网 CSV 批量导入、重置并导出密码）、管理员账号、周次信息（含一键生成整学期）
...
> 管理员登录后，建议先在「周次」页用「**一键生成本学期安排**」建好整学期进度（未配置周次时首页会提示「尚未配置」，不会报错，配置后自动切到当前周）；
> 再到「人员 → 学生信息 / 管理员信息」添加学生 / 管理员账号（也可从课程网导出的 CSV **批量导入学生**）。
> 添加 / 导入时会自动生成随机初始密码并只显示一次，分发给本人后由其首次登录修改。
...
「开启时间 / 提交截止 / 评论结束」三个时间点，系统按 **7 天一周**切分并自动平移生成（章节默认 Lec.周次，可在下方表格微调为区间）。
...
- 开发配置默认 `DEBUG=True`、内置开发用 `SECRET_KEY`、`ALLOWED_HOSTS=['localhost','127.0.0.1']` —— **请勿直接用于公网生产**。
  上线前请用环境变量覆盖：`DJANGO_DEBUG=false`、`DJANGO_SECRET_KEY=<强随机值>`、`DJANGO_ALLOWED_HOSTS=<逗号分隔域名>`。
- 周次表驱动所有页面：未配置任何周次时，系统进入「尚未配置」空态并给出友好提示（**不会报错**）；
  配置后自动以「进行中的周」为当前周，并在新增 / 一键生成 / 删除周次后自动切到初始周。
  建议开课前先在「周次管理」用「一键生成本学期安排」建好整学期进度。
- 业务数据通过「学号 / 用户主键」等整数引用（**有意不设外键**，以保留历史）；
  删除账号后其历史问题仍保留，提问人显示为「未知」。
